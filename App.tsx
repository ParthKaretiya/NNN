...
const AnalysisWorkspace: React.FC = () => {
  ...
  // ✅ ADDED: This Logic watches the status and rotates the loading text
  useEffect(() => {
    let interval: any;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-100px)]">
        ...
        {/* ✅ CHANGED: The main heading now updates dynamically */}
        <h2 className="text-2xl font-bold text-white mb-2 h-8 transition-all duration-300">
          {loadingText}
        </h2>

        <p className="text-zinc-500 text-center max-w-sm px-4">
          SentinelAI is modeling attacker pathways and generating secure fixes. 
          Please wait for the secure sandbox to finalize.
        </p>
      </div>
    );
  }

    return () => clearInterval(interval); // Clean up on unmount or status change
  }, [status]);
  ...
  if (status === AnalysisStatus.ANALYZING) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-100px)]">
        ...
        {/* ✅ UPDATED: Now uses the dynamic loadingText state */}
...
const AnalysisResults: React.FC<{ result: any, onReset: () => void }> = ({ result, onReset }) => {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {/* Sidebar List of Vulnerabilities */}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-800">
                        ...
                        <span className="text-sm font-bold text-cyan-500">
                          {Math.round((selectedVuln?.confidence || 0) * 100)}%
                        </span>
                      </div>
                      <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-800">
                    ...
                    <div className="px-4 py-2 bg-zinc-900 border-b border-zinc-800 flex justify-between items-center shrink-0">
                      ...
                      >
                        {copied ? (
                          ...
                          <>
                            <Download className="w-3 h-3" /> Copy Fix
                          </>
                        )}
                      </button>
                    </div>
                  ...
                  <div className="space-y-6">
                    <section className="bg-red-500/5 border border-red-500/20 p-4 rounded-xl">
                      ...
                      <p className="text-sm text-red-200/70 leading-relaxed italic">
                        "{selectedVuln?.attackerLogic || "No specific logic returned."}"
                      </p>
                    </section>
                    <section>
                    ...
                    <section>
                      <h5 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2">Simulated Exploit Payload</h5>
                      <div className="p-3 bg-zinc-950 border border-red-500/30 rounded-lg font-mono text-xs text-red-400 break-all">
                        {selectedVuln?.simulatedPayload || "No payload generated."}
                      </div>
                    </section>
                  </div>
